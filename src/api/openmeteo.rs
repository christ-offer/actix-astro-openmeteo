...
async fn openmeteo(open_meteo_request: web::Json<OpenMeteoRequest>) -> Result<impl Responder> {
    ...
    let url: String = format!("https://nominatim.openstreetmap.org/search?q={}&format=json", city);
    println!("url: {}", url);
    ...
    impl Daily {
        pub fn calculate_stats(&self) -> Stats {
            Stats {
                ...
                temperature_2m_mean: Self::calculate_stat_map(&self.temperature_2m_mean, &self.time),
                apparent_temperature_max: Self::calculate_stat_map(&self.apparent_temperature_max, &self.time),
                apparent_temperature_min: Self::calculate_stat_map(&self.apparent_temperature_min, &self.time),
                apparent_temperature_mean: Self::calculate_stat_map(&self.apparent_temperature_mean, &self.time),
                shortwave_radiation_sum: Self::calculate_stat_map(&self.shortwave_radiation_sum, &self.time),
                precipitation_sum: Self::calculate_stat_map(&self.precipitation_sum, &self.time),
                precipitation_hours: Self::calculate_stat_map(&self.precipitation_hours, &self.time),
                ...
                et0_fao_evapotranspiration: Self::calculate_stat_map(&self.et0_fao_evapotranspiration, &self.time),
        ...
        fn calculate_stat_map(data: &Vec<Option<f64>>, times: &Vec<Option<String>>) -> StatMap {
            ...
            StatMap {
                min: MinMaxMean {
                    time: min.map(|(i, _)| times[i].clone().unwrap()).unwrap_or("".to_string()),
                    value: min.map(|(_, v)| v).unwrap_or(0.0),
                },
                max: MinMaxMean {
                    time: max.map(|(i, _)| times[i].clone().unwrap()).unwrap_or("".to_string()),
